		...
		<div class="flex flex-col sm:flex-row justify-between items-stretch gap-4">
			...
			<div class="flex gap-4 sm:gap-6 text-3xl uppercase font-thin tracking-wider">
				<div class="whitespace-nowrap">
					<span class="font-semibold">{{ filteredSongs.length }}</span>
					...
					<td class="hidden 2xl:table-cell">
						<label class="relative">
							...
							<select v-model="filter.tag" class="w-full pl-8">
								<option v-for="key in tags" :key="key" :value="key">
									{{ t(`tag.${key}`) }}
								</option>
							</select>
						</label>
					...
					<td class="hidden md:table-cell">
						<label class="relative">
							...
							<select v-model="filter.language" class="w-full pl-8">
								<option v-for="key in languages" :key="key" :value="key">
									{{ t(`language.${key}`) }}
								</option>
							</select>
						</label>
			...
			<tbody>
				...
				>
					...
					>
						<div class="truncate">
							{{ song.entity.title }}
							<span class="text-blade-500 ml-2">{{ song.entity.subtitle }}</span>
						</div>
					</td>
					<td
						...
						@click="router.push({ name: 'song-show', params: { id: song.entity.slug }})"
					>
						<div class="truncate">{{ song.entity.authors?.join(' | ') ?? '' }}</div>
					</td>
